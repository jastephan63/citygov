...
| `index.html` | The start page of the website (<https://jastephan63.github.io/citygov/>): links to the dashboard, the dossiers and the guided forms, with the data date. Generated by `scripts/build_index.py`. |
| `dashboard.html` | The dashboard, one self-contained file of just under 40 MB (about 5 MB over the network): per service its Verfahren, Formulare, data fields, legal bases, standards, handling rules and digitalisation status. |
...
then double-click the saved `dashboard.html`. It runs without internet in
any current browser; only the typefaces fall back to system fonts. The links
to dossiers and Formular files need the whole repository (next option).
...
Unzip it and double-click `index.html` (start page) or `dashboard.html`.
